...
with open('2019/Day11_input.txt', 'r') as f:
    ...
    while True:
        color = 0
        if position in visitedLocations:
            color = visitedLocations[position]
        output = []
        #color
        program.run([color], output)
        #rotation
        program.run([], output)
        if len(output) >= 1:
            visitedLocations[position] = output[0]
        if len(output) >= 2:
            forward = (forward[1], -forward[0]) if output[1] == 0 else (-forward[1], forward[0])
            position = (position[0] + forward[0], position[1] + forward[1])
        #move forward
        if program.halted:
            break
    ...
    if part == 2:
        ...
        for y in range(height):
            row = "".join(["#" if (x + minX, y + minY) in visitedLocations and visitedLocations[(x + minX, y + minY)] == 1 else " " for x in range(width)])
            print(row)